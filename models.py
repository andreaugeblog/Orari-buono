...
class HistoryStats:
    ...
    # numero di giorni consecutivi lavorati IMMEDIATAMENTE prima di reference
    # (serve a contare i giorni di fila a cavallo tra storico e nuovo periodo)
    work_tail: dict[str, int] = field(default_factory=dict)
    ...
    def from_assignments(
        ...
        # Coda di giorni consecutivi lavorati subito prima di reference.
        # Guardiamo all'indietro fino a 14 giorni (ne bastano 6 per la regola,
        # ma 14 rende il dato robusto). Ci si ferma al primo giorno non lavorato.
        by_emp_day = {(a.employee_id, a.giorno): a for a in assignments}
        for eid in {a.employee_id for a in assignments}:
            tail = 0
            for i in range(1, 15):
                a = by_emp_day.get((eid, reference - timedelta(days=i)))
                if a is not None and a.is_lavoro:
                    tail += 1
                else:
                    break
            stats.work_tail[eid] = tail
        return stats
