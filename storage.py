...
def save_schedule(data_inizio: date, num_settimane: int, stato: str,
    ...
    """Salva un orario generato come UNA VOCE PER SETTIMANA.
    Una generazione multi-settimana viene spezzata in righe schedules distinte
    (una per lunedi). Rigenerando una settimana gia' presente, la si sovrascrive
    invece di duplicarla. Le assegnazioni restano sovrascritte per (dip, giorno)."""
    from datetime import timedelta
    with _connect() as conn:
        with conn.cursor() as cur:
            # crea/aggiorna una riga schedules per ciascuna settimana coperta
            week_id: dict[date, int] = {}
            for w in range(num_settimane):
                lunedi = data_inizio + timedelta(days=7 * w)
                cur.execute(
                    """
                    INSERT INTO schedules (data_inizio, num_settimane, stato)
                    VALUES (%s, 1, %s)
                    ON CONFLICT (data_inizio) DO UPDATE SET
                      stato=EXCLUDED.stato, num_settimane=1, generato_il=now()
                    RETURNING id
                    """,
                    (lunedi, stato),
                )
                week_id[lunedi] = cur.fetchone()["id"]

            # ogni assegnazione viene legata alla settimana (lunedi) a cui appartiene
            for a in assignments:
                g = a["giorno"]
                gd = g if isinstance(g, date) else date.fromisoformat(g)
                lunedi = gd - timedelta(days=gd.weekday())  # lunedi della sua settimana
                sid = week_id.get(lunedi)
                cur.execute(
                    """
                    ...
                    ON CONFLICT (employee_id, giorno) DO UPDATE SET
                      ...
                      aggiornato_il=now()
                    """,
                    (sid, a["employee_id"], a["giorno"], a["turno"]),
                )
        conn.commit()
    # ritorna l'id della prima settimana (compatibilita')
    return week_id.get(data_inizio, 0)
...
def pulisci_duplicati_schedules() -> dict:
    """Rimuove i record schedules duplicati per data_inizio, tenendo il piu'
    recente. Reimposta num_settimane=1 (nuovo modello una-voce-per-settimana)
    e aggiunge il vincolo UNIQUE se assente. Idempotente: si puo' richiamare."""
    with _connect() as conn:
        with conn.cursor() as cur:
            # quanti duplicati ci sono prima
            cur.execute("SELECT COUNT(*) AS tot FROM schedules")
            prima = cur.fetchone()["tot"]
            # elimina i duplicati tenendo, per ogni data_inizio, il generato_il piu' recente
            cur.execute(
                """
                ...
                WHERE s.data_inizio = s2.data_inizio
                  AND (s.generato_il < s2.generato_il
                       OR (s.generato_il = s2.generato_il AND s.id < s2.id))
                """
            )
            # normalizza num_settimane a 1 (modello una-voce-per-settimana)
            cur.execute("UPDATE schedules SET num_settimane = 1 WHERE num_settimane <> 1")
            # aggiunge il vincolo UNIQUE se non c'e' gia'
            cur.execute(
            ...
            if not cur.fetchone():
                try:
                    ...
                    pass
            cur.execute("SELECT COUNT(*) AS tot FROM schedules")
            dopo = cur.fetchone()["tot"]
        conn.commit()
    return {"prima": prima, "dopo": dopo, "rimossi": prima - dopo}
