...
def genera(req: GenerateRequest):
    ...
    return GenerateResponse(
        ...
        violazioni=getattr(res, "violazioni", []),
...
@app.get("/pulisci-storico")
def get_pulisci_storico():
    """Rimuove i record duplicati dallo storico (una sola volta basta).
    Tiene la versione piu' recente di ogni settimana."""
    if not storage.is_configured():
        return {"persistenza": False, "messaggio": "persistenza non configurata"}
    risultato = storage.pulisci_duplicati_schedules()
    return {"persistenza": True, **risultato}
...
# ============================================================================
#  STATISTICHE (turni e ferie, anche per dipendenti archiviati)
# ============================================================================
@app.get("/dipendenti-tutti")
def get_dipendenti_tutti():
    """Tutti i dipendenti, inclusi gli archiviati (per la tab statistiche)."""
    if not storage.is_configured():
        return {"persistenza": False, "employees": []}
    rows = storage.get_employees(include_archiviati=True)
    return {"persistenza": True, "employees": rows}


@app.get("/statistiche")
def get_statistiche(start: str, end: str):
    """Statistiche turni per tutti i dipendenti (attivi e archiviati) nel periodo."""
    if not storage.is_configured():
        return {"persistenza": False, "stats": {}, "employees": []}
    d0 = date.fromisoformat(start)
    d1 = date.fromisoformat(end)
    stats = storage.statistiche_turni(d0, d1)
    emps = storage.get_employees(include_archiviati=True)
    return {
        "persistenza": True,
        "stats": stats,
        "employees": [
            {"id": e["id"], "nome": e["nome"], "archiviato": e["archiviato"]}
            for e in emps
        ],
    }


@app.get("/statistiche-ferie")
def get_statistiche_ferie(anno: int):
    """Giorni di ferie per dipendente in un anno solare."""
    if not storage.is_configured():
        return {"persistenza": False, "ferie": {}, "anni": []}
    ferie = storage.statistiche_ferie(anno)
    return {
        "persistenza": True,
        "ferie": ferie,
        "anni": storage.anni_con_ferie(),
    }
