...
W_WEEKDAY_INTER = 40
W_EVENING_FAIR = 10
W_CONSEC_6 = 500       # 6 giorni lavorativi consecutivi (tollerato se serve)
W_CONSEC_7 = 3000      # 7+ giorni consecutivi (da evitare quasi sempre)
...
class ScheduleSolver:
    ...
    # ------------------------------------------- SOFT: giorni consecutivi
    def _all_worked_var(self, var_terms, const_ones, name):
        """BoolVar y = 1 sse TUTTI i giorni della finestra sono lavorati.
        var_terms: lista di BoolVar (giorni del periodo).
        const_ones: numero di giorni storici nella finestra, tutti = 1
                    (le finestre con uno storico = 0 non arrivano qui).
        Modellazione di un AND reificato, gestendo le costanti separatamente
        per evitare somme miste variabili/interi."""
        y = self.model.NewBoolVar(name)
        # upper bound: y <= ogni variabile
        for t in var_terms:
            self.model.Add(y <= t)
        # lower bound: se tutte le variabili sono 1 (e le costanti lo sono già),
        # allora y deve essere 1. sum(var_terms) raggiunge len(var_terms) solo
        # quando tutte valgono 1.
        n = len(var_terms)
        if n == 0:
            # finestra interamente storica e tutta lavorata -> y = 1 fisso
            self.model.Add(y == 1)
        else:
            self.model.Add(y >= sum(var_terms) - (n - 1))
        return y

    def _soft_consecutive_work(self):
        """Penalizza sequenze di lavoro troppo lunghe (>=6, >=7 giorni di fila),
        considerando anche la coda di giorni gia' lavorati prima del periodo
        (work_tail letto dallo storico). 1-5 giorni: nessuna penalita'."""
        for e in self.employees:
            tail = 0
            if self.history and getattr(self.history, "work_tail", None):
                tail = self.history.work_tail.get(e.id, 0) or 0
            tail = max(0, min(6, tail))  # per finestre fino a 7 bastano 6 storici
            # bit storici: (6 - tail) giorni non lavorati + tail giorni lavorati
            history_bits = [0] * (6 - tail) + [1] * tail
            # sequenza continua: storico (costanti) + variabili del periodo
            work_vars = [self.work[(e.id, d)] for d in self.days]
            seq = history_bits + work_vars
            offset = len(history_bits)  # indice del primo giorno reale

            for end_idx in range(offset, len(seq)):
                real_day = self.days[end_idx - offset]
                # finestra di 6 giorni che termina in end_idx
                if end_idx - 5 >= 0:
                    win = seq[end_idx - 5:end_idx + 1]
                    consts = [t for t in win if isinstance(t, int)]
                    vars_ = [t for t in win if not isinstance(t, int)]
                    # se uno storico nella finestra e' 0, impossibile tutti lavorati
                    if all(c == 1 for c in consts):
                        y6 = self._all_worked_var(vars_, len(consts), f"c6_{e.id}_{end_idx}")
                        self.obj_terms.append(W_CONSEC_6 * y6)
                        self.penalties.append(("consec_6", e, real_day, y6))
                # finestra di 7 giorni
                if end_idx - 6 >= 0:
                    win = seq[end_idx - 6:end_idx + 1]
                    consts = [t for t in win if isinstance(t, int)]
                    vars_ = [t for t in win if not isinstance(t, int)]
                    if all(c == 1 for c in consts):
                        y7 = self._all_worked_var(vars_, len(consts), f"c7_{e.id}_{end_idx}")
                        self.obj_terms.append(W_CONSEC_7 * y7)
                        self.penalties.append(("consec_7", e, real_day, y7))
    ...
    def _raccogli_violazioni(self, solver):
        ...
        out = []
        # giorni con consec_7 attivo (per persona): sopprimono il consec_6 ridondante
        giorni_consec7 = set()
        for tipo, e, d, var in self.penalties:
            if tipo == "consec_7" and solver.Value(var) > 0:
                giorni_consec7.add((e.id, d))
        for tipo, e, d, var in self.penalties:
            val = solver.Value(var)
            if val <= 0:
                continue
            # salta il messaggio "6 giorni" se sullo stesso giorno c'e' gia' "7 giorni"
            if tipo == "consec_6" and (e.id, d) in giorni_consec7:
                continue
            ...
            if tipo == "fixed_off":
                ...
                out.append(f"{e.nome}: 6 giorni di lavoro consecutivi fino a {giorno} (accettato per la copertura).")
            elif tipo == "consec_7":
                out.append(f"{e.nome}: 7+ giorni di lavoro consecutivi fino a {giorno} — situazione da evitare, controlla organico/ferie.")
