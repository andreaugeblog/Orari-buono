    ...
    def _c_due_giorni_liberi(self):
        ...
        for e in self.employees:
            for w in range(self.num_days // 7):
                ...
                # Se ferie/liberi-fissi gia' coprono >=2 giorni, NON devono
                # esserci liberi aggiuntivi: la persona lavora in tutti gli altri
                # giorni disponibili (salvo giorno forte, che puo' richiamarla).
                # BUG FIX: prima si usciva con `continue` lasciando il solver
                # libero di assegnare liberi extra -> >2 giorni liberi.
                if n_forced_free >= 2:
                    for d in week_days:
                        wd = Weekday(d.weekday())
                        if self._is_ferie(e.id, d):
                            continue
                        if wd in e.giorni_liberi_fissi and self._giorno_forte(d) is None:
                            continue
                        # ogni altro giorno (non ferie, non libero fisso) = lavoro
